import { LayoutDashboard, FileText, Users, BarChart2, Settings, Plus } from "lucide-react"
...
function isActive(navId: string, view: View) {
  if (navId === 'invoices') return view === 'invoices' || view === 'invoice-new' || view === 'invoice-detail'
  return navId === view
}
...
export function Sidebar({ view, onNav, onNewInvoice }: SidebarProps) {
  return (
    <aside className="w-[232px] shrink-0 bg-sidebar flex flex-col h-screen sticky top-0 py-5 px-3.5">
      {/* Logo */}
      <div className="flex items-center gap-2.5 px-2 pb-[18px] border-b border-sidebar-border mb-3.5">
        ...
        <span className="text-[15px] font-semibold text-sidebar-foreground tracking-tight">Invome</span>
      </div>

      {/* New invoice */}
      <button
        onClick={onNewInvoice}
        className="w-full mb-[18px] h-9 flex items-center justify-center gap-2 rounded-sm border border-white/20 text-sidebar-foreground text-sm font-medium hover:bg-sidebar-accent transition-colors cursor-pointer"
      >
        <Plus size={16} />
        New invoice
      </button>
      ...
      <div className="mt-auto pt-3.5 border-t border-sidebar-border flex items-center gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-[13px] font-medium text-sidebar-foreground truncate">Ọlámidé Adébáyọ̀</div>
          <div className="text-[11px] text-sidebar-foreground/50">BrightLab Studios</div>
        </div>
      </div>
    </aside>
  )
