...
export function Dashboard({ onNav, onOpenInvoice }: DashboardProps) {
  const outstanding = invoices
    ...
    .reduce((s, i) => s + (i.currency === 'NGN' ? i.amount : i.amount * 1500), 0)
  ...
  return (
    <div>
      <PageHeader title="Dashboard" subtitle="An overview of your invoicing activity." />
      ...
      <div className="grid grid-cols-4 gap-3.5 mb-6">
        ...
        <StatCard
          ...
          sub={<span><b className="text-success-fg">↑ 18%</b> vs last month</span>}
          Icon={CheckCircle2}
          tint="green"
        />
        <StatCard label="Overdue" value={overdueCount} sub="1 over 14 days" Icon={AlertTriangle} />
        ...
        <Card>
          ...
          <div className="px-5 pb-5 pt-1">
            {invoices.filter(i => i.status === 'overdue').map(inv => {
              ...
              const days = Math.floor((new Date('2026-04-18').getTime() - new Date(inv.due).getTime()) / 86400000)
              return (
                <div
                  key={inv.id}
                ...
                >
                  <div>
                    <div className="text-[13px] font-medium text-foreground">{client.company}</div>
                    <div className="text-xs text-warning-fg mt-0.5">{days} days past due</div>
                  </div>
                  <div className="text-right">
                    <div className="t-num text-sm font-semibold text-foreground">
                      {formatMoney(inv.amount, inv.currency)}
                    </div>
                    <div className="t-num text-[11px] text-muted-foreground mt-0.5">{inv.id}</div>
                  </div>
                </div>
              )
            })}
          ...
          <tbody>
            {recent.map(inv => {
              ...
              return (
                ...
                >
                  <Td><span className="t-num font-medium">{inv.id}</span></Td>
                  <Td>
                    {client.company}
                  ...
                  <Td><span className="t-num text-secondary-foreground">{formatDate(inv.due)}</span></Td>
                </tr>
              )
            })}
          </tbody>
...
function StatCard({ label, value, sub, Icon, tint }: StatCardProps) {
  return (
    ...
    >
      ...
      <div className="t-num text-xs text-secondary-foreground mt-1.5">{sub}</div>
    </div>
  )
}
  ...
  return (
    <div className="px-5 pb-5 pt-4">
      <div className="t-num text-[28px] font-semibold text-foreground tracking-tight mb-[18px]">
        ...
        <span className="text-[13px] font-normal text-secondary-foreground ml-2.5">total billed</span>
      </div>
      ...
      <div className="flex gap-[18px] mt-2">
        {data.map((d, i) => (
          <div key={i} className="flex-1 text-center t-num text-[11px] text-muted-foreground font-medium">
            {d.month}
          </div>
        ))}
      </div>
