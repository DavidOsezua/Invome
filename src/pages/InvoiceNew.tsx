...
export function InvoiceNew({ onCancel, onSave }: InvoiceNewProps) {
  ...
  const [notes, setNotes] = useState('Thank you for the work. Payment due within 14 days of issue.')
  ...
  const updateLine = (id: number, k: keyof LineItem, v: string) =>
    setLines(lines.map(l => l.id === id ? { ...l, [k]: k === 'desc' ? v : Number(v) || 0 } : l))
  ...
  return (
    <div className="grid gap-5 items-start" style={{ gridTemplateColumns: '1fr 320px' }}>
      <div>
        <PageHeader title="New invoice" subtitle="Invoice #INV-0143 · auto-generated" />
          ...
          <div className="p-5">
            ...
            {client && (
              <div className="mt-3 p-3.5 bg-canvas rounded-sm text-[13px] text-secondary-foreground leading-relaxed">
                <div className="text-foreground font-medium">{client.company}</div>
        ...
        <Card className="mb-4">
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr>
                <Th>Description</Th>
                <Th align="right" className="w-[90px]">Qty</Th>
                <Th align="right" className="w-[140px]">Unit price</Th>
            ...
            <tbody>
              {lines.map(l => (
                <tr key={l.id}>
                  ...
                  <Td align="right">
                    <span className="t-num">{formatMoney(l.qty * l.unit, client.currency)}</span>
                  </Td>
                  <Td>
                    <button
        ...
        <Card className="mb-4">
          ...
          <div className="p-5 grid grid-cols-2 gap-4">
            <Field label="Invoice number"><Input value="INV-0143" readOnly /></Field>
            <Field label="Issue date"><Input type="date" defaultValue="2026-04-18" /></Field>
            <Field label="Due date"><Input type="date" defaultValue="2026-05-02" /></Field>
            ...
            <Field label="Currency">
              <Select value={client.currency} onChange={() => {}} options={[
                { value: 'NGN', label: 'NGN — ₦' },
                { value: 'USD', label: 'USD — $' },
              ]} />
            </Field>
            <div className="col-span-2">
              <Field label="Notes (printed on invoice)">
                <Textarea value={notes} onChange={e => setNotes(e.target.value)} className="h-[72px]" />
              </Field>
            </div>
          </div>
        </Card>
        ...
        <Card>
          ...
          <div className="p-5">
            <SumRow label="Subtotal">
              <span className="t-num">{formatMoney(subtotal, client.currency)}</span>
            </SumRow>
            ...
            <div className="flex flex-col gap-2 mt-4">
              ...
              <Button variant="outline" className="w-full"><Eye size={16} />Preview</Button>
              <Button variant="ghost" className="w-full" onClick={onCancel}>Save draft</Button>
            </div>
          </div>
        </Card>
...
function SumRow({ label, children, big }: SumRowProps) {
  return (
    <div className="flex justify-between items-center py-1.5">
      <span className={big ? "text-[14px] font-semibold text-foreground" : "text-[13px] text-secondary-foreground"}>
        {label}
      </span>
      {children}
    </div>
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-5 py-[18px] border-b border-surface-muted">
          <h3 className="m-0 text-base font-semibold text-foreground">Add new client</h3>
          <button onClick={onClose} className="bg-transparent border-0 text-muted-foreground cursor-pointer p-1">
            <X size={18} />
          </button>
        </div>
        <div className="p-5 flex flex-col gap-3.5">
          <Field label="Company name"><Input placeholder="e.g. BrightLab Studios" /></Field>
          <Field label="Contact name"><Input placeholder="e.g. Adaeze Okafor" /></Field>
          <Field label="Email"><Input placeholder="client@company.com" /></Field>
          <Field label="Default currency">
            <Select value="NGN" onChange={() => {}} options={[
        ...
        <div className="flex justify-end gap-2 px-5 py-3.5 border-t border-surface-muted">
          ...
          <Button variant="default" onClick={onClose}>Save client</Button>
        </div>
      </div>
    </div>
  )
